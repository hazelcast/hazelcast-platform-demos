...
STATEFULSET_NAME=transaction-monitor-kafka-broker
POSTGRES_NAME=transaction-monitor-postgres
PULSAR_NAME=transaction-monitor-pulsar
IPLIST=""
POSTGRESADDRESS=""
PULSARLIST=""
...
echo "  name: transaction-monitor-configmap" >> $TMPFILE
echo "data:" >> $TMPFILE
...
echo "    ID=\`echo \$MY_POD_NAME | sed s/transaction-monitor-kafka-broker-//\`" >> $TMPFILE
...
#echo "    INTERNAL=\${MY_POD_NAME}.transaction-monitor-kafka-broker.default.svc.cluster.local:\${INTERNAL_PORT}" >> $TMPFILE
