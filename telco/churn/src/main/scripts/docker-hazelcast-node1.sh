...
CMD="docker run -e MY_BOOTSTRAP_SERVERS=$MY_BOOTSTRAP_SERVERS -e MY_KUBERNETES_ENABLED=false -e JAVA_ARGS=-Dhazelcast.local.publicAddress=${HOST_IP}:5701 -p 5701:5701 --name=${MODULE} --network=${PROJECT} ${DOCKER_IMAGE}"
#echo $CMD
