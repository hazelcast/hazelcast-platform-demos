...
CMD="docker run -e MY_BOOTSTRAP_SERVERS=$MY_BOOTSTRAP_SERVERS -e MY_INITSIZE=1 -e MY_KUBERNETES_ENABLED=false -e JAVA_ARGS=-Dhazelcast.local.publicAddress=${HOST_IP}:5703 -p 5703:5701 --name=${MODULE}3 --network=${PROJECT} ${DOCKER_IMAGE}"
#echo $CMD
